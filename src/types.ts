...
export interface RustFaustModule {
    ...
    /**
     * Optional on older Rust compiler modules.
     *
     * Returns a text-result handle containing the complete diagnostics-v2
     * report for a compiler failure.
     */
    faust_wasm_result_get_error_diagnostics?(handle: number): number;
    /**
     * Optional on older Rust compiler modules.
     *
     * Returns a text-result handle containing warnings/remarks retained by a
     * successful compilation.
     */
    faust_wasm_result_get_diagnostics?(handle: number): number;
...
export interface LibFaustWasm {
    ...
    /**
     * Complete structured report for the most recent compiler failure.
     *
     * Optional because historical Emscripten modules expose text only.
     */
    getErrorDiagnosticsAfterException?():
        import('./FaustDiagnostics').FaustDiagnosticReport | null;

    /**
     * Complete warnings/remarks report retained by the most recent successful
     * compilation. Optional on historical compiler modules.
     */
    getDiagnostics?():
        import('./FaustDiagnostics').FaustDiagnosticReport | null;
